...
namespace FarseerPhysics.Dynamics.Contacts
{
    ...
    public class Contact
    {
        ...
        internal void MyUpdate(ContactManager contactManager, Dictionary<int, HashSet<Body>> dictionary)
        {
            Body bodyA = FixtureA.Body;
            Body bodyB = FixtureB.Body;
            ...
            else
            {
                ...
                {
                    //_contactManager.ContactUpdate(new CollideEventArg(bodyA.BodyId, bodyB.BodyId));
                    bodyA.Awake = true;
                    bodyB.Awake = true;
            ...
            if (wasTouching == false)
            {
                if (touching)
                {
                    if (dictionary.ContainsKey(bodyA.BodyId))
                        dictionary[bodyA.BodyId].Add(bodyB);
                    else
                        dictionary.Add(bodyA.BodyId, new HashSet<Body>() { bodyB });


                    if (dictionary.ContainsKey(bodyB.BodyId))
                        dictionary[bodyB.BodyId].Add(bodyA);
                    else
                        dictionary.Add(bodyB.BodyId, new HashSet<Body>() { bodyA });
